(() => {
  ...
  function getFlatTheme(data) {
    const themes = {
      "white-studio": { bg: "#ffffff", ink: "#11110f", titleFont: 'Arial, "Microsoft YaHei", sans-serif', titleWeight: 900, titleSize: 88, visual: "ring" },
  ...
  function getFlatLayout(style) {
    ...
    const layouts = {
      ...
      workshop: { kicker: { x: .18, y: .045, w: .29, h: .03 }, title: { x: .27, y: .13, w: .49, h: .14 }, subtitle: { x: .25, y: .235, w: .44, h: .04 }, visual: { x: .78, y: .14, w: .22, h: .16 }, date: { x: .80, y: .055, w: .28, h: .045, align: "right" }, time: { x: .76, y: .25, w: .32, h: .03, align: "right" }, venue: { x: .76, y: .29, w: .36, h: .055, align: "right" }, body: { x: .27, y: .31, w: .48, h: .06 }, courses: { x: .50, y: .49, w: .88, h: .30 }, organizer: { x: .25, y: .965, w: .44, h: .025 }, qr: { x: .91, y: .63, w: .075, h: .06 } }
    };
  ...
  function drawFlatPoster(c, data, W, H, s, seed) {
    ...
    if (data.style === "workshop") order.splice(order.length - 2, 0, "courses");
    order.forEach((id) => {
  ...
  function drawFlatBackground(c, data, W, H, theme, seed) {
    ...
    if (data.style === "white-studio") {
      ...
      const margin = W * .028, gridCols = 12, cell = (W - margin * 2) / gridCols;
      const mosaicTop = H * .69, gridRows = Math.max(5, Math.floor((H - mosaicTop - H * .045) / cell));
      const rng = mulberry32(seed);
      c.strokeStyle = theme.ink; c.lineWidth = Math.max(2, W * .002); c.beginPath(); c.moveTo(margin, mosaicTop); c.lineTo(W - margin, mosaicTop); c.stroke();
      c.font = `${cell * .72}px "Segoe UI Emoji", "Apple Color Emoji", sans-serif`; c.textAlign = "center"; c.textBaseline = "middle";
  ...
  function drawFlatVisual(c, data, w, h, theme, seed) {
    ...
    if (image) {
      ...
      c.fillStyle = "#ffffff"; c.fillRect(-w / 2, -h / 2, w, h);
      c.fillStyle = alpha(theme.accent, .18);
      for (let row = -1; row <= 1; row++) for (let col = -2; col <= 2; col++) { c.beginPath(); c.arc(col * w * .16, row * h * .23, Math.min(w, h) * .055, 0, Math.PI * 2); c.fill(); }
      c.font = `${Math.min(w, h) * .40}px "Segoe UI Emoji", "Apple Color Emoji", sans-serif`; c.textAlign = "center"; c.textBaseline = "middle"; c.fillText(data.motifEmoji || "👀", 0, 0);
    } else {
      c.fillStyle = variant === "electric" ? "#9e4dff" : variant === "blue" ? "#b9d8ff" : variant === "layout" ? alpha(theme.accent, .24) : "#ecebe6";
      c.fillRect(-w / 2, -h / 2, w, h);
      c.fillStyle = theme.ink;
      if (variant === "ring" || variant === "quiet" || variant === "atlas") {
        c.beginPath(); c.arc(0, 0, Math.min(w, h) * .25, 0, Math.PI * 2); c.fill();
        c.fillStyle = theme.bg; c.beginPath(); c.arc(0, 0, Math.min(w, h) * .15, 0, Math.PI * 2); c.fill();
      } else {
        c.fillRect(-w * .34, -h * .08, w * .68, h * .16);
        c.fillStyle = theme.accent; c.fillRect(-w * .10, -h * .38, w * .20, h * .76);
      }
    }
    c.restore();
    c.strokeStyle = theme.ink; c.lineWidth = Math.max(2, w * .008);
    if (variant === "quiet" || variant === "ring" || variant === "atlas") { c.beginPath(); c.arc(0, 0, Math.min(w, h) * .46, 0, Math.PI * 2); c.stroke(); }
    else if (variant !== "electric" && variant !== "workshop") c.strokeRect(-w / 2, -h / 2, w, h);
  }
    ...
    imageAssets.forEach((asset, index) => {
      ...
      item.innerHTML = `
        <div class="asset-thumb">
          <img alt="${escapeHtml(asset.name)}" src="${asset.dataUrl}">
